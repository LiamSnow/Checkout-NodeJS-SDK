...
class PayPalHttpClient extends paypalhttp.HttpClient {
  ...
  constructor(environment, refreshToken) {
    ...
    this.addInjector(function (req) {
      ...
      req.headers["sdk_version"] = version;
   ...
   */
  getUserAgent() {
    return 'PayPalSDK/PayPal-node-SDK ' + version + ' (node ' + process.version + '-' + process.arch + '-' + process.platform + '; OpenSSL ' + process.versions.openssl + ')';
  }
